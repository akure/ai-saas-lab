...
Welcome to your AI-assisted, pro-level Git workflow! Since the `pro-git-workflow` skill is installed in your `.agents/skills` directory, I will automatically enforce naming conventions, commit formats, and pre-flight tests for this repository. 
...
Instead of typing commands in the terminal yourself, just ask me in plain English. I will ensure the branch name matches the strict `<type>/<description>` format.
...
> **Prompt:** *"Add these changes to the previous commit."*
> **My Action:** I will test the code and propose `git commit --amend --no-edit`.

---

## 3. Handling Pull Requests (PRs)
I can write world-class PR descriptions using the `gh` CLI tool, populating the Context, Implementation Details, and Audit Checklist automatically based on my knowledge of the code we just wrote.

> **Prompt:** *"Push this branch and create a PR against main."*
> **My Action:** 
> 1. Run `git push -u origin <branch>`.
> 2. Run `gh pr create --title "..." --body "..."` using our strict enterprise PR template.

---

## 4. Resolving Merge Conflicts Safely
Merge conflicts can be dangerous. Our skill ensures I don't accidentally overwrite your business logic. 

> **Prompt:** *"Pull the latest from main and rebase my branch on top of it."*
> **My Action:** I will run `git fetch` and `git rebase origin/main`. 
> 
